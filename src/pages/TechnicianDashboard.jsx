...
export default function TechnicianDashboard() {
  ...
  const handleExportRepairPDF = async (repair) => {
    try {
      // Try to fetch complete repair details if needed
      const repairData = repair.RepairOrder ? repair : {
        ...repair,
        RepairOrder: orders?.find(o => o.id === repair.order_id)
      }
      
      exportRepairDetailsToPDF(repairData)
    } catch (error) {
      console.error("[v0] Error exporting repair PDF:", error)
                  ...
                  <div key={repair.id} className="card" style={{ padding: "16px" }}>
                    ...
                    <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                      ...
                      <button
                        className="btn btn--ghost"
                        style={{ padding: "4px 12px", fontSize: "0.875rem" }}
                        onClick={() => handleExportRepairPDF(repair)}
                      >
                        PDF
                      </button>
                    </div>
